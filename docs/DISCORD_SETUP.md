...
6. Under **Redirects**, add the exact URLs used by the backend:
   - Local: `http://localhost:4000/api/auth/discord/callback`
   - Production: `https://YOUR-API-DOMAIN/api/auth/discord/callback`
...
Keep the frontend and API on the same parent domain when possible, for example
`maskborn.xyz` and `api.maskborn.xyz`, so secure session cookies work consistently.
Never put the Discord client secret in frontend variables or commit it.
